...
const Analytics = () => {
  ...
  const noRequestsOverall = async () => {
    let sendjson = await fetch("/cslabs/noofrequests");
  ...
  const requestsWithSolutions = async () => {
    let solvedjson = await fetch("/cslabs/requestsWithSolutions");
    solved = await solvedjson.json();
    console.log(solved);
  };

  const fetchEducators = async () => {
    let educatorsjson = await fetch("/cslabs/retrieveteachers");
    fetchedEducators = await educatorsjson.json();
    console.log(fetchedEducators);
  };

  const requestsSolvedByEducator = async () => {
    let requestsSolvedEducatorjson = await fetch('/cslabs/requestspereducator', {
        headers: {
            'Content-type': 'application/json'
        },
        method: 'POST',
